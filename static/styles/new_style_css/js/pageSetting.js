...
function levelMoodSubmit() {
    ...
    if (arrayError != "") {
        $("#levelMoodSubmit").addClass("ajax-error");
...
function addActionNewSubmit() {
    ...
    if (arrayError != "") {
        $("#addNewActionSubmit").addClass("ajax-error");
        ...
        return;
         ...
         $.ajax({
            ...
            if (response['error'] != "") {
        $("#addNewActionSubmit").addClass("ajax-error").removeClass("ajax-succes");
        $("#addNewActionSubmit").html("Już jest taka akcja o takiej nazwie");
            }
            else {
                $("#addNewActionSubmit").addClass("ajax-succes");
...
function addNewPosition() {
        ...
        var td2 = "<td  >";
        var td2End = '</td>';
        var td3 = "<td ><input type='text' name='portion[]' class='form-control' id='trPlanedInput" + (positionPlaned+2) + "'></td>";
        var td3End = "</td>" ;
        var td4 = "<td  >";
        var td4End = "</td>" ;
        ...
        $('#tablePlaned').append("<tr> <td > pozycja " + (positionPlaned+2) + "</td>" + td2 + selectNameResult2 + td2End + td3 + td3End   + td4 +  result + td4End + "</tr>");
        positionPlaned++;
}
...
function selectedGroupSubstance(id,index) {
    ...
    if ($("#divGroupGroup_" + id + ":first").hasClass("settings-group-all-2")) {
        $("#divGroupGroup_" + id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
        arrayGroupSubstance.push(id);
    }
    else {
        var i = arrayGroupSubstance.indexOf(id);
        arrayGroupSubstance.splice(i,1);
        $("#divGroupGroup_" + id).removeClass("settings-group-add-selected").addClass("settings-group-all-2");
...
function selectedSubstanceChangeMainSetValue(data,lenght) {
    ...
    for (var i = 0;i < lenght;i++) {
        ...
        if ($("#divSubstanceSubstanceChange_" + data[i].id).length==1) {
            $("#divSubstanceSubstanceChange_" + data[i].id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
...
function selectedProductChangeMainSetValue(data,lenght) {
    ...
    for (var i = 0;i < lenght;i++) {
        if ($("#divSubstanceProductChange_" + data[i].id).length==1) {
            ...
            $("#divSubstanceProductChange_" + data[i].id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
...
function selectedProductChangeMainValue(id,index) {
    if ($("#divSubstanceProductChange_" + id + ":first").hasClass("settings-group-all-2")) {
        $("#divSubstanceProductChange_" + id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
        $("#divActionPercent_" + id).removeClass("settings-group-hidden-percent").addClass("showPercentExecuting");
        arraySubstanceProductChange.push(id);
    }
    else {
        var i = arraySubstanceProductChange.indexOf(id);
        arraySubstanceProductChange.splice(i,1);
        $("#divSubstanceProductChange_" + id).removeClass("settings-group-add-selected").addClass("settings-group-all-2");
        $("#divActionPercent_" + id).removeClass("showPercentExecuting").addClass("settings-group-hidden-percent");

    }   
}
function selectedSubstanceChangeMainValue(id,index) {
    
    if ($("#divSubstanceSubstanceChange_" + id + ":first").hasClass("settings-group-all-2")) {
        $("#divSubstanceSubstanceChange_" + id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
        ...
        $("#divSubstanceSubstanceChange_" + id).removeClass("settings-group-add-selected ").addClass("settings-group-all-2");
...
function selectedProductProduct(id,index) {
    ...
    if ($("#divSubstanceSubstance_" + id + ":first").hasClass("settings-group-all-2")) {
        $("#divSubstanceSubstance_" + id).removeClass("settings-group-all-2").addClass("settings-group-add-selected");
        $("#divSubstanceSubstancePercent_" + id).removeClass("settings-group-hidden-percent").addClass('active');
        arraySubstanceProduct.push(id);
    }
    else {
        var i = arraySubstanceProduct.indexOf(id);
        arraySubstanceProduct.splice(i,1);
        $("#divSubstanceSubstancePercent_" + id).addClass("settings-group-hidden-percent").removeClass('active');
        $("#divSubstanceSubstance_" + id).removeClass("settings-group-add-selected").addClass("settings-group-all-2");
...
function editSubstanceSubmit() {
 var arrayError = "";
    ...
    if (arrayError != "") {
        $("#updateSubstanceDiv").addClass("ajax-error");
...
function editPlanedSubmitFunction(url) {
    ...
    if (checkErrorPlanedSelect !=  "" || checkErrorPlanedInput !=  "") {
        $("#updatePlanedDiv").addClass("ajax-error");
...
function editProductSubmit() {
 var arrayError = "";
    ...
    if (arrayError != "") {
        $("#updateProductDiv").addClass("ajax-error");
...
function addProductNewSubmit() {
 var arrayError = "";
 $("#addNewProductSubmit").removeClass("ajax-error");
    if ($("input[name='nameProduct']").val() == "") {
        arrayError += "Uzupełnij nazwe Produktu<br>";
    }

    if (arrayError != "") {
        $("#addNewProductSubmit").addClass("ajax-error");
...
function addGroupNewSubmit() {
 var arrayError = "";
    ...
    if (arrayError != "") {
        $("#addNewGroupSubmit").addClass("ajax-error");
        ...
        return;
         ...
         $.ajax({
            ...
            if (response['error'] != "") {
        $("#addNewGroupSubmit").addClass("ajax-error").removeClass("ajaxSucces");
        $("#addNewGroupSubmit").html("Już jest taka grupa o takiej nazwie");
            }
            else {
                $("#addNewGroupSubmit").addClass("ajax-succes");
